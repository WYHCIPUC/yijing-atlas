...
assert.match(html, /class="relation-layers"[\s\S]*class="star-map-sequence"[^>]*aria-label="星河观察层级"/);
assert.deepEqual([...html.matchAll(/data-star-stage="([^"]+)"/g)].map((match) => match[1]), ['galaxy', 'cluster', 'hexagram', 'yao']);
...
assert.match(html, /六爻为卫星、卦为恒星，并按下卦汇成八个星团/);
