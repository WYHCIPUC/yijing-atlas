...
test('预缓存只包含首屏核心模块，功能数据按访问缓存', () => {
  const core = [
    './js/main.js?v=28', './js/data-loader.js', './js/evolution-lab.js', './js/evolution-state.js',
    './js/guaxu-wheel.js', './js/modes/guaxu-mode.js',
    './js/motion-system.js', './js/render.js', './js/star-map.js',
...
test('缓存资源不再维护重复查询版本号', () => {
  ...
  assert.deepEqual(revisioned.sort(), ['./js/main.js?v=28', './styles/main.css?v=28']);
});

test('页面结构与脚本样式采用同代更新策略', () => {
  assert.match(serviceWorker, /const CACHE_NAME = 'yijing-atlas-v28'/);
