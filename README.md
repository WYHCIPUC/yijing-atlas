...
易象图谱是一个默认零后端、零运行时依赖的《易经》学习网站。它以六十四卦关系星图为入口，整合学习路径、卦序、复习、考评、传统黄历知识和占筮文化演示，并支持安装为 PWA 与离线再次访问。

## 功能概览

- 六十四卦关系星图、全文检索和卦爻辞详情
- 八卦、六十四卦、十翼、象数与历法日用五阶学习路径
- 日课、小试、随机抽查、复讲、阶段考评与综合掌握度
- 连续学习、错题回练、本地复习卡及 JSON 备份
...
普通用户可在 GitHub 的 [Releases](https://github.com/WYHCIPUC/yijing-atlas/releases) 页面下载 `yijing-atlas-*-windows-x64.exe`。双击后程序会启动本地服务并自动打开浏览器；保持启动窗口开启，关闭窗口即可退出。该版本已内置网站与 Node.js 运行时，不需要另行安装依赖。

维护者在 Windows 上可运行以下命令生成并验证发布文件：
...
产物位于 `dist/`，同时生成 `SHA256SUMS.txt` 供下载后校验。推送 `v*` 标签会触发 GitHub Actions 自动构建并发布。
...
学习数据仅存于当前浏览器，请在“学习 → 数据”中定期导出备份。复讲默认使用典籍要点自评；如需智能评阅，应另行部署安全的服务端代理，浏览器和仓库中不得保存模型密钥。黄历、占筮和体用解释只用于传统文化学习，不构成专业建议；日期按设备本地时区计算。内容来源和校对边界见 [docs/CONTENT-SOURCES.md](docs/CONTENT-SOURCES.md)，发布步骤见 [docs/RELEASE-CHECKLIST.md](docs/RELEASE-CHECKLIST.md)。
